...
if (Utils.findGlobal('C_CurrencyInfo', 'GetCurrencyListInfo') == nil) then
...
local function readCurrencyTable ()
  ...
  while (index <= listSize) do
    ...
    if (info == nil) then
      Strings.printOneTimeMessage('Could not check currencies as another addon seems to be interfering with the currency pane');
      break;
    end
...
Events.onOnce('FIRST_FRAME_RENDERED', function ()
  currencyTable = readCurrencyTable();
  Events.on('CURRENCY_DISPLAY_UPDATE', handleCurrency);
end);
...
Tests.addTest('currency', function ()
